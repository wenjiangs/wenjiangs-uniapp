<template>
  <view class="page">
    ...
    <view class="publishComment" v-if="showCF">
      ...
      <view class="pubText">
        <textarea fixed="true" :placeholder="plaText" v-model="newCommentContent" cursor-spacing="32rpx"
          show-confirm-bar="false" placeholder-class="plaClass"></textarea>
      </view>
    </view>
    <view class="content">
      <scroll-view scroll-y="true" :style="'height:' + svHeight + 'px;'" @scrolltolower="loadMore" @scroll="onPageScrollS">
        <view class="docsItem">
        ...
        <view class="docsMenu" v-if="cType==2">
          <view class="wjList">
            <view class="wjListCell" @click="wjRouterPush({path:'single', query:{id: item.ID}})" v-for="(item, index) in post"
              :key="index">
        ...
        <view class="docsComment" v-if="cType==3">
          <loop-comment :comments="comment" :noMore="noCommentMore" :userID="userInfo.ID*1" :isLoading="isCommentLoading"
            :comment_count="docsSingle.comment_count*1"></loop-comment>
        </view>
      </scroll-view>
    </view>
  </view>
